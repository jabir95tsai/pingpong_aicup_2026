"""V18 Pipeline — V14 + hierarchical (soft-decoded) point head.

Same architecture as V14 EXCEPT the 10-class pointId head is replaced by three
soft heads:
  - HEAD_VALID: binary classifier for (point != 0). Trained on ALL train rows.
  - HEAD_DEPTH: 3-class (short / half / long). Trained on ON-GRID SUBSET only.
  - HEAD_SIDE : 3-class (FH / mid / BH).        Trained on ON-GRID SUBSET only.

OOF reconstruction (per fold, per row):
  oof_pt[k=0] = 1 - p_valid
  oof_pt[k]   = p_valid * p_depth[DEPTH_OF[k]] * p_side[SIDE_OF[k]]   for k=1..9
Each row is then renormalised to sum to 1.0 (corrects for product-of-marginals
approximation error). DO NOT softmax over the 9 reconstructed terms — that
breaks the probability scale (Codex 2026-05-05).

The action model and server model are unchanged from V14.

Codex sign-off (2026-05-05):
- ON-GRID SUBSET training (NOT sample_weight=0 placeholders).
- Soft probabilistic recombination (NOT hard decode like V12-era hier).
- Reconstruction happens INSIDE the CV loop (not post-hoc cross-fold).
- cls0 F1 gate: v18 cls0 F1 must be ≥ V14 cls0 F1 − 0.01 to be promoted to zoo.
"""
...
N_POINT        = 10
N_DEPTH        = 3         # short(0) / half(1) / long(2)
N_SIDE         = 3         # FH(0) / mid(1) / BH(2)

ACTION_EVAL_LABELS = list(range(15))   # 15-class macro F1 for action
POINT_EVAL_LABELS  = list(range(10))

# Hierarchy mapping for the 9 on-grid pointId classes.
DEPTH_OF = {1: 0, 2: 0, 3: 0, 4: 1, 5: 1, 6: 1, 7: 2, 8: 2, 9: 2}
SIDE_OF  = {1: 0, 4: 0, 7: 0, 2: 1, 5: 1, 8: 1, 3: 2, 6: 2, 9: 2}
DEPTH_VEC = np.array([0, 0, 0, 0, 1, 1, 1, 2, 2, 2], dtype=np.int32)  # idx 0 unused
SIDE_VEC  = np.array([0, 0, 1, 2, 0, 1, 2, 0, 1, 2], dtype=np.int32)  # idx 0 unused


def hier_reconstruct(p_valid: np.ndarray, p_depth: np.ndarray,
                     p_side: np.ndarray) -> np.ndarray:
    """Soft probabilistic recombination of the 3 heads into a 10-class pt prob.

    p_valid : shape (n,)        probability that point != 0
    p_depth : shape (n, 3)      P(depth | point != 0)
    p_side  : shape (n, 3)      P(side  | point != 0)

    Returns an (n, 10) array, row-normalised to sum to 1.
    """
    n = len(p_valid)
    out = np.zeros((n, N_POINT), dtype=np.float32)
    out[:, 0] = (1.0 - p_valid).astype(np.float32)
    for k in range(1, N_POINT):
        d = DEPTH_OF[k]
        s = SIDE_OF[k]
        out[:, k] = (p_valid * p_depth[:, d] * p_side[:, s]).astype(np.float32)
    # Row renormalise (corrects the product-of-marginals approximation error).
...
def main():
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits):
        ...
        # ══════════════════════════════════════════════════════════════════════
        # PASS B — V18 hierarchical POINT head (valid + depth + side)
        #          with action probs as extra stacking features
        # ══════════════════════════════════════════════════════════════════════
        if use_stack:
            X_tr_ext      = np.hstack([X_tr,     pa_tr_15])
            X_tr_aug_ext  = np.hstack([X_tr_aug, pa_tr_aug_15])
        ...
        # Labels for the three heads.
        y_valid_aug   = (y_p_aug != 0).astype(np.int32)
        y_valid_val   = (y_p_val != 0).astype(np.int32)
        on_grid_aug   = (y_p_aug != 0)
        on_grid_val   = (y_p_val != 0)

        X_tr_on_ext   = X_tr_aug_ext[on_grid_aug]
        y_p_on        = y_p_aug[on_grid_aug]
        sw_p_on       = sw_p[on_grid_aug]
        y_depth_on    = np.array([DEPTH_OF[int(c)] for c in y_p_on], dtype=np.int32)
        y_side_on     = np.array([SIDE_OF[int(c)]  for c in y_p_on], dtype=np.int32)

        X_val_on_ext  = X_val_ext[on_grid_val]
        y_p_val_on    = y_p_val[on_grid_val]
        y_depth_val_on = np.array([DEPTH_OF[int(c)] for c in y_p_val_on], dtype=np.int32)
        y_side_val_on  = np.array([SIDE_OF[int(c)]  for c in y_p_val_on], dtype=np.int32)

        # Per-bucket sample weights derived from POINT_CW (rebalanced for 3 classes).
        # depth: short = mean(POINT_CW[1,2,3]) etc.; side analogous.
        DEPTH_CW = np.array([
            np.mean([POINT_CW[1], POINT_CW[2], POINT_CW[3]]),
            np.mean([POINT_CW[4], POINT_CW[5], POINT_CW[6]]),
            np.mean([POINT_CW[7], POINT_CW[8], POINT_CW[9]]),
        ], dtype=np.float32)
        SIDE_CW = np.array([
            np.mean([POINT_CW[1], POINT_CW[4], POINT_CW[7]]),
            np.mean([POINT_CW[2], POINT_CW[5], POINT_CW[8]]),
            np.mean([POINT_CW[3], POINT_CW[6], POINT_CW[9]]),
        ], dtype=np.float32)
        sw_depth_on = DEPTH_CW[y_depth_on]
        sw_side_on  = SIDE_CW[y_side_on]

        # ── HEAD VALID (binary on point != 0; trained on ALL train rows) ───────
        lgb_v_p = dict(n_estimators=n_boost, learning_rate=0.04,
                       num_leaves=63, max_depth=7, min_child_samples=10,
                       subsample=0.8, colsample_bytree=0.7,
                       objective="binary", metric="binary_logloss",
                       random_state=seed, n_jobs=-1, verbose=-1)
        lgb_v = lgb.train(lgb_v_p,
            lgb.Dataset(X_tr_aug_ext, label=y_valid_aug.astype(np.float32)),
            valid_sets=[lgb.Dataset(X_val_ext, label=y_valid_val.astype(np.float32))],
            callbacks=[lgb.early_stopping(es_rounds, verbose=False),
                       lgb.log_evaluation(-1)])
        ...
        xgb_v.fit(X_tr_aug_ext, y_valid_aug,
                  eval_set=[(X_val_ext, y_valid_val)], verbose=False)
        p_valid_val = (lgb_v.predict(X_val_ext) +
                       xgb_v.predict_proba(X_val_ext)[:, 1]) / 2.0

        # ── HEAD DEPTH (3-class; trained on ON-GRID SUBSET only) ───────────────
        lgb_d_p = dict(n_estimators=n_boost, learning_rate=0.04,
                       num_leaves=63, max_depth=7, min_child_samples=8,
                       subsample=0.8, colsample_bytree=0.7,
                       reg_alpha=0.1, reg_lambda=1.0,
                       objective="multiclass", metric="multi_logloss",
                       num_class=N_DEPTH, random_state=seed,
                       n_jobs=-1, verbose=-1)
        lgb_d = lgb.train(lgb_d_p,
            lgb.Dataset(X_tr_on_ext, label=y_depth_on, weight=sw_depth_on),
            valid_sets=[lgb.Dataset(X_val_on_ext, label=y_depth_val_on)],
            callbacks=[lgb.early_stopping(es_rounds, verbose=False),
                       lgb.log_evaluation(-1)])
        xgb_d = xgb.XGBClassifier(
            n_estimators=n_boost, learning_rate=0.04, max_depth=7,
            subsample=0.8, colsample_bytree=0.7, min_child_weight=3,
            objective="multi:softprob", num_class=N_DEPTH,
            eval_metric="mlogloss", early_stopping_rounds=es_rounds,
            random_state=seed, n_jobs=-1, verbosity=0, tree_method="hist")
        xgb_d.fit(X_tr_on_ext, y_depth_on, sample_weight=sw_depth_on,
                  eval_set=[(X_val_on_ext, y_depth_val_on)], verbose=False)
        # Predict on FULL val set (depth probs are conditional on point != 0; the
        # reconstruction weights them by p_valid).
        p_depth_val = (lgb_d.predict(X_val_ext) +
                       pad_proba(xgb_d.predict_proba(X_val_ext),
                                 xgb_d.classes_, N_DEPTH)) / 2.0

        # ── HEAD SIDE (3-class; trained on ON-GRID SUBSET only) ────────────────
        lgb_si_p = dict(n_estimators=n_boost, learning_rate=0.04,
                       num_leaves=63, max_depth=7, min_child_samples=8,
                       subsample=0.8, colsample_bytree=0.7,
                       reg_alpha=0.1, reg_lambda=1.0,
                       objective="multiclass", metric="multi_logloss",
                       num_class=N_SIDE, random_state=seed,
                       n_jobs=-1, verbose=-1)
        lgb_si = lgb.train(lgb_si_p,
            lgb.Dataset(X_tr_on_ext, label=y_side_on, weight=sw_side_on),
            valid_sets=[lgb.Dataset(X_val_on_ext, label=y_side_val_on)],
            callbacks=[lgb.early_stopping(es_rounds, verbose=False),
                       lgb.log_evaluation(-1)])
        xgb_si = xgb.XGBClassifier(
            n_estimators=n_boost, learning_rate=0.04, max_depth=7,
            subsample=0.8, colsample_bytree=0.7, min_child_weight=3,
            objective="multi:softprob", num_class=N_SIDE,
            eval_metric="mlogloss", early_stopping_rounds=es_rounds,
            random_state=seed, n_jobs=-1, verbosity=0, tree_method="hist")
        xgb_si.fit(X_tr_on_ext, y_side_on, sample_weight=sw_side_on,
                  eval_set=[(X_val_on_ext, y_side_val_on)], verbose=False)
        p_side_val = (lgb_si.predict(X_val_ext) +
                      pad_proba(xgb_si.predict_proba(X_val_ext),
                                xgb_si.classes_, N_SIDE)) / 2.0

        # OOF reconstruction (INSIDE the fold loop — no cross-fold contamination).
        pp_hier_val = hier_reconstruct(p_valid_val, p_depth_val, p_side_val)
        f1_p_val = point_macro_f1(y_p_val, pp_hier_val)
        print(f"  [Point-hier]  F1={f1_p_val:.4f}  "
              f"(valid_acc={(((p_valid_val>0.5)==y_valid_val).mean()):.3f})")
        oof_pt[val_mask] = pp_hier_val
        # Keep oof_pt_bin for downstream compatibility (= p_valid).
        oof_pt_bin[val_mask] = p_valid_val

        # Test point predictions: fold-averaged action probs as stacking features.
        if use_stack:
            X_test_ext = np.hstack([X_test, test_act15_acc * len(splits) / (fold + 1)])
        else:
            X_test_ext = X_test

        p_valid_test = (lgb_v.predict(X_test_ext) +
                        xgb_v.predict_proba(X_test_ext)[:, 1]) / 2.0
        p_depth_test = (lgb_d.predict(X_test_ext) +
                        pad_proba(xgb_d.predict_proba(X_test_ext),
                                  xgb_d.classes_, N_DEPTH)) / 2.0
        p_side_test  = (lgb_si.predict(X_test_ext) +
                        pad_proba(xgb_si.predict_proba(X_test_ext),
                                  xgb_si.classes_, N_SIDE)) / 2.0
        pp_hier_test = hier_reconstruct(p_valid_test, p_depth_test, p_side_test)
        test_pt_acc  += pp_hier_test / len(splits)
        test_bin_acc += p_valid_test / len(splits)
    ...
    # ─── V18 cls0 regression gate (Codex 2026-05-05) ─────────────────────────
    # Compare v18 cls0 F1 against V14 (v14_seed0 OOF) baseline. Hierarchical
    # decoupling can silently damage cls0; this gate ensures we don't blend a
    # v18 with worse off-grid prediction than V14.
    print("\n  [V18 cls0 gate]")
    v14_pt_path = os.path.join(os.path.dirname(SUBMISSION_DIR),
                               "oof_predictions", "v14_seed0_oof_pt.npy")
    v14_y_path = os.path.join(os.path.dirname(SUBMISSION_DIR),
                              "oof_predictions", "v14_seed0_oof_y_pt.npy")
    short_classes = (1, 2, 3)
    if os.path.exists(v14_pt_path) and os.path.exists(v14_y_path):
        v14_pt = np.load(v14_pt_path)
        v14_y  = np.load(v14_y_path)
        v14_pred = np.argmax(v14_pt, axis=1)
        v14_f1s = f1_score(v14_y, v14_pred, labels=POINT_EVAL_LABELS,
                           average=None, zero_division=0)
        v18_cls0 = float(pf1s[0])
        v14_cls0 = float(v14_f1s[0])
        delta = v18_cls0 - v14_cls0
        gate = (delta >= -0.01)
        print(f"    V14 cls0 F1 (baseline)  = {v14_cls0:.4f}")
        print(f"    V18 cls0 F1             = {v18_cls0:.4f}")
        print(f"    delta                   = {delta:+.4f}")
        print(f"    gate (delta >= -0.01)   = {'PASS' if gate else 'FAIL'}")
        v18_short = float(np.mean([pf1s[i] for i in short_classes]))
        v14_short = float(np.mean([v14_f1s[i] for i in short_classes]))
        d_short = v18_short - v14_short
        gate_short = (d_short >= 0.03)
        print(f"    V14 short F1 (mean of cls 1/2/3) = {v14_short:.4f}")
        print(f"    V18 short F1 (mean of cls 1/2/3) = {v18_short:.4f}")
        print(f"    delta short              = {d_short:+.4f}")
        print(f"    gate (delta short >= +0.03) = {'PASS' if gate_short else 'FAIL'}")
        if not gate:
            print(f"    >>> V18 cls0 GATE FAILED — DO NOT BLEND v18 INTO ZOO. <<<")
        if not gate_short:
            print(f"    >>> V18 short F1 GATE FAILED — structural change ineffective. <<<")
    else:
        print(f"    v14_seed0 baseline OOF NOT FOUND at {v14_pt_path}")
        print(f"    Falling back to absolute F1_p >= 0.235 gate.")
        v18_f1p = float(f1_p_oof)
        gate_abs = (v18_f1p >= 0.235)
        print(f"    V18 F1_p = {v18_f1p:.4f}  gate (>= 0.235) = {'PASS' if gate_abs else 'FAIL'}")
    ...
    # Final test point predictions (V18 hierarchical re-fit using LAST fold's
    # three heads on the FULL fold-averaged action stacking features).
    # NOTE: blend_two_stage is intentionally NOT used here — the hierarchical
    # reconstruction already factorises the off-grid mass via (1 - p_valid).
    if use_stack:
        X_test_ext_final = np.hstack([X_test, test_act15_acc])
        p_valid_final = (lgb_v.predict(X_test_ext_final) +
                         xgb_v.predict_proba(X_test_ext_final)[:, 1]) / 2.0
        p_depth_final = (lgb_d.predict(X_test_ext_final) +
                         pad_proba(xgb_d.predict_proba(X_test_ext_final),
                                   xgb_d.classes_, N_DEPTH)) / 2.0
        p_side_final  = (lgb_si.predict(X_test_ext_final) +
                         pad_proba(xgb_si.predict_proba(X_test_ext_final),
                                   xgb_si.classes_, N_SIDE)) / 2.0
        test_pt_acc_final = hier_reconstruct(p_valid_final, p_depth_final,
                                             p_side_final)
        test_bin_acc_final = p_valid_final
    ...
    # V18: skip blend_two_stage (hierarchical already encodes off-grid mass).
    # Optionally re-renormalise after the average above.
    test_pt_acc_norm = test_pt_acc / np.maximum(
        test_pt_acc.sum(axis=1, keepdims=True), 1e-12)
    test_pt_t  = test_pt_acc_norm ** (1.0 / t_p)
