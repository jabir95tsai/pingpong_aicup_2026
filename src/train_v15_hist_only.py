"""V15 Pipeline — V12 + features_v10 (joint serve-receive priors))

Identical architecture to V10 (Two-Pass Action→Point Stacking) but uses
features_v10 which adds 25 joint serve-receive prior features on top of V6:
...
def main():
    ...
    parser.add_argument("--tag",      type=str, default="v15_hist_only",
    ...
    t_start = time.time()
    print("=" * 70)
    print(f"V12 PIPELINE (V10 + V7 grammar) {'(SMOKE)' if is_smoke else ''}")
    print(f"  aug={use_aug}  folds={n_folds}  n_boost={n_boost}  stack={use_stack}  "
          f"skip_cb={skip_cb}  es={es_rounds}")
    ...
    from features_v10 import (compute_global_stats_v10, build_features_v10,
                              get_feature_names_v10)
    # Wrappers so existing call sites that use v6 kwarg names still work
    compute_global_stats_v6 = compute_global_stats_v10
    get_feature_names_v6     = get_feature_names_v10
    def build_features_v6(df, is_train, global_stats_v6, raw_df=None):
        # ABLATION: hist freq + streak only (NO player profile)
        return build_features_v10(df, is_train=is_train,
                                  global_stats_v10=global_stats_v6,
                                  raw_df=raw_df,
                                  include_player_profile=False,
                                  include_hist_freq=True,
                                  include_streak=True)
