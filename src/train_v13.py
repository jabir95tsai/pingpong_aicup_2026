"""V13 Pipeline — V10 + Action-Point Grammar Priors (features_v8)

Identical architecture to V10 (Two-Pass Action→Point Stacking) but uses
features_v8 which adds 24 grammar prior features on top of V6:
...
def main():
    ...
    t_start = time.time()
    print("=" * 70)
    print(f"V13 PIPELINE (V10 + V8 point-grammar) {'(SMOKE)' if is_smoke else ''}")
    ...
    compute_global_stats_v6 = compute_global_stats_v8
    get_feature_names_v6     = get_feature_names_v8
    def build_features_v6(df, is_train, global_stats_v6, raw_df=None):
        return build_features_v8(df, is_train=is_train,
