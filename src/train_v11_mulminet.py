...
# Expected counts for the test-history aug parquet (mirrors V16 guards).
...
def train_epoch(model, loader, optimizer, scaler, act_loss_fn, pt_loss_fn,
    ...
    for batch in loader:
        ...
        # The line below is intentionally 0: we never include is_aug==1 rows in the
        # server BCE. Counter exists so a regression test (or grep) can fail loudly
        # if a future edit re-broadens the BCE to include aug rows.
        aug_rows_in_server_loss += 0
...
def main():
    ...
    parser.add_argument("--tag",            type=str, default="v11_mulminet")
    parser.add_argument("--seed",           type=int, default=None,
                        help="(R-026) Override RANDOM_SEED for snapshot/seed-ensemble diversity. "
                             "Default None uses config.RANDOM_SEED=42.")
    parser.add_argument("--aux-lambda",     type=float, default=0.2,
                        help="(R-018) MuLMINet aux-loss weight. Default 0.2; "
                             "sweep on full run. Setting 0.0 disables aux losses (= V11 baseline).")
    parser.add_argument("--include-old-test", type=str, default=None,
                        help="(NEW 2026-05-13) Path to old test.csv to include as additional training data. "
                             "Per AICUP organizers' 2026-05-13 announcement allowing old test as training data. "
                             "When set, rows from this file are concatenated to train_df with full labels "
                             "(including real SGP). They participate in normal training (not is_aug=1).")
    parser.add_argument("--folds",          type=int, default=N_FOLDS)
    parser.add_argument("--max-folds",      type=int, default=0,
                        help="If > 0, run only the first N folds (full-epoch). "
                             "Used for stop-gate dry runs (e.g. v11_small Fold-1 "
                             "OV ≥ 0.305 gate per REVIEW_QUEUE.md R-002).")
    ...
    parser.add_argument("--aug-parquet",    type=str, default="",
                        help="Path to test_history_pairs.parquet (P6: V11 + test-history "
                             "augmentation). When set, aug rows are concatenated to the "
                             "training set per fold, contribute to action+point losses, "
                             "and are MASKED OUT of the server BCE (Codex requirement).")
    ...
    seed_actual = args.seed if args.seed is not None else RANDOM_SEED
    torch.manual_seed(seed_actual)
    np.random.seed(seed_actual)
    print(f"  random seed = {seed_actual}")
    ...
    if args.include_old_test:
        # NEW 2026-05-13: include old test.csv as additional training data
        # per AICUP announcement allowing it. Old test has full labels including SGP.
        old_test = pd.read_csv(args.include_old_test)
        n_before = len(raw_train)
        # Verify old test has all required columns
        ...
        n_after = len(raw_train)
        print(f"  [include-old-test] Added {n_after - n_before} rows from {args.include_old_test} "
              f"({len(old_test['rally_uid'].unique())} rallies, {len(old_test['match'].unique())} matches)")
        print(f"  Total train rows: {n_before} -> {n_after}")
    ...
    print(f"  Train samples: {n_train}  Test samples: {len(test_samples)}  ({time.time()-t0:.1f}s)")

    # ── Optional: load V16 test-history aug parquet (P6) ──────────────────────
    aug_samples = []
    if args.aug_parquet:
        print(f"\n--- Loading aug parquet (P6 V11 + test-history aug) ---")
        print(f"    Path: {args.aug_parquet}")
        t_aug = time.time()
        aug_raw = pd.read_parquet(args.aug_parquet)

        # Hard guards mirroring V16
        ...
        assert len(aug_raw) == expected_aug_rows, \
            f"GUARD FAIL: expected {expected_aug_rows} aug rows, got {len(aug_raw)}."
        print(f"    Aug raw rows: {len(aug_raw)}  (expected {expected_aug_rows})")

        # Same cleaning as V16: STRIKE_ID_MAP + player_map + numberGame clip.
        ...
        # build_samples uses serverGetPoint as label; we keep the -1 placeholder and
        # MASK it out of server BCE in train_epoch (P6 server-mask, Codex requirement).
        aug_samples = build_samples(aug_raw, is_train=True, n_players=n_players)
        for s in aug_samples:
            s["is_aug"] = 1
        assert len(aug_samples) == expected_aug_pairs, \
            f"GUARD FAIL: expected {expected_aug_pairs} aug pairs, got {len(aug_samples)}."
        print(f"    Aug samples: {len(aug_samples)}  (expected {expected_aug_pairs})  "
              f"[{time.time()-t_aug:.1f}s]")
        print(f"    NO_TRUE_TEST_SGP_USED = True (server BCE masked for is_aug==1)")

    # Concatenate: train_samples[0..n_train-1] are real; aug_samples after that.
    all_samples = train_samples + aug_samples
    n_aug = len(aug_samples)
    print(f"  Total samples: {len(all_samples)} (real={n_train}, aug={n_aug})")

    # Rally→match mapping for GroupKFold (real train only; aug rows split separately).
    ...
    # True labels for OOF evaluation (real train only — aug rows never enter OOF).
    ...
    # OOF containers (sized to REAL train only — aug rows never produce OOF rows).
    ...
    # GroupKFold splits ONLY over real train samples (P6: aug rows never enter OOF).
    # Aug indices are concatenated to each fold's training set after the split.
    ...
    if is_smoke:
        ...
        # Full-epoch fold-limited mode for stop-gate dry runs (R-002).
        # Unlike --smoke, this keeps the full epoch budget per fold.
        splits = splits[:args.max_folds]
        print(f"  --max-folds {args.max_folds} active: running first "
              f"{len(splits)} of {n_folds} folds with full epoch budget.")
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits):
        ...
        if n_aug > 0:
            print(f"  Train rows this fold: {len(tr_samps)} (real={len(tr_idx)}, aug={n_aug})")
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if aug_in_srv != 0:
                ...
                raise AssertionError(
                    f"P6 GUARD FAIL: aug_rows_in_server_loss={aug_in_srv} (must be 0).")
    ...
    print(f"  OOF: {n_oof}/{n_train} ({100*n_oof/n_train:.0f}%)")
