"""V12 Pipeline — V10 + Action-Point Grammar Priors (features_v7)

Identical architecture to V10 (Two-Pass Action→Point Stacking) but uses
features_v7 which adds 24 grammar prior features on top of V6:
...
def main():
    ...
    parser.add_argument("--skip-cb",  action="store_true",
                        help="Skip CatBoost models (saves ~50% time)")
    ...
    from features_v7 import (compute_global_stats_v7, build_features_v7,
                              get_feature_names_v7)
    # Wrappers so existing call sites that use v6 kwarg names still work
    compute_global_stats_v6 = compute_global_stats_v7
    get_feature_names_v6     = get_feature_names_v7
    def build_features_v6(df, is_train, global_stats_v6, raw_df=None):
        return build_features_v7(df, is_train=is_train,
                                  global_stats_v7=global_stats_v6,
                                  raw_df=raw_df)
