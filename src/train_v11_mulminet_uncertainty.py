...
class RallyTransformer(nn.Module):
    def __init__(self, d_model=192, n_heads=8, n_layers=4, dropout=0.1,
        ...
        # R-022: Kendall & Gal uncertainty-weighted MTL for 3 MAIN tasks
        # (action/point/server). Aux losses keep fixed λ weight.
        # Initialize log-vars at 0 (uniform weight 0.5 per task).
...
def train_epoch(model, loader, optimizer, scaler, act_loss_fn, pt_loss_fn,
    ...
    for batch in loader:
        ...
        with autocast():
            ...
            # R-022: Kendall & Gal uncertainty-weighted MTL for main tasks.
            # Replaces fixed 0.4/0.4/0.2 with learnable log-variance per task.
            # Aux tasks keep fixed λ weight (their relative balance is already
            # handled by the λ hyperparameter).
            ...
            loss = loss_a_w + loss_p_w + loss_s_w + aux_lambda * loss_aux_sum
...
def main():
    ...
    parser.add_argument("--tag",            type=str, default="v11_mulminet_uncertainty")
